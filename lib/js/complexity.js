...
var exec = require('child_process').exec,
  config = require('../../config/jscomplexity.json');
...
function validateLines(loc) {
  return validate(loc, config.maxLines, 'LONG_FILE');
}
...
function validateCyclomaticComplexity(cyclomatic) {
  return validate(cyclomatic, config.maxCyclomatic, 'HIGH_CYCLOMATIC_COMPLEXITY');
}
...
function validateDifficulty(difficulty) {
  return validate(difficulty, config.maxHDifficulty, 'HIGH_DIFFICULTY');
}
...
function validateVolume(volume) {
  return validate(volume, config.maxHVolume, 'HIGH_VOLUME');
}
...
function validateEffort(effort) {
  return validate(effort, config.maxHEffort, 'HIGH_EFFORT');
}
...
function validateBugs(bugs) {
  return validate(bugs, config.maxBugDensity, 'HIGH_BUG_DENSITY');
}
...
function run(paths, callback) {
  var command = 'node ' + __dirname + '/../../node_modules/complexity-report/src/index.js -f json ';
  exec(command + paths.join(' '), function (error, stdout) {
    var reports = parse(JSON.parse(stdout).reports);
